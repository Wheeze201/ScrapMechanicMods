...
function CreativeGame.client_onCreate( self )
	...
	sm.game.bindChatCommand( "/import", { { "string", "name", false } }, "cl_onChatCommand", "Imports blueprint from survival $SURVIVAL_DATA/LocalBlueprints/<name>.blueprint" )
...
function CreativeGame.sv_importCreation( self, params )
	...
	if status == false then
		self.network:sendToClients("client_showMessage", "An error occurred (note: survival-only parts cannot be imported to creative)")
	end
end
