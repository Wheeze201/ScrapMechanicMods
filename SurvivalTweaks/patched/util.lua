...
-- [SurvivalTweaks] noFallDamage is a trailing optional argument, passed only by
-- BasePlayer. Every other caller (the bots in Survival/Scripts/game/units) passes fewer
-- arguments, so it arrives as nil for them and their behaviour is unchanged.
function CharacterCollision( self, other, vCollisionPosition, vPointVelocitySelf, vPointVelocityOther, vCollisionNormal, maxhp, velDiffThreshold, velocityBuffer, velocityBufferIndex, fallDamageMultiplier, noFallDamage )
	...
	-- [SurvivalTweaks] Zeroing fallDamage alone does NOT disable fall damage: the line
	-- below falls through to collisionDamage, and landing on the ground is itself a
	-- collision, so the collision branch simply deals the damage the fall branch no
	-- longer does. Both have to go, and only for a landing on something that is not
	-- itself moving - the same test the block just above uses. So a creation dropped on
	-- your head still hurts, and specialCollisionDamage (spinning sawblades) is left
	-- alone deliberately. fallTumbleTicks is untouched, so hard landings still ragdoll.
