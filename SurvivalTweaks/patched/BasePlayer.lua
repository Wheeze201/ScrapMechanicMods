...
-- [SurvivalTweaks] Vanilla 1.0 BasePlayer.lua with the fall damage toggle
-- (one changed line in server_onCollision; see the comment there)
...
function BasePlayer.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal  )
	...
	-- [SurvivalTweaks] trailing arg: g_disableFallDamage is set per world in
	-- SurvivalGame.lua and read here, server side, the same way vanilla passes
	-- g_enableCollisionTumble around. Creative never sets it, so there it is nil.
	local damage, tumbleTicks, tumbleVelocity, impactReaction = CharacterCollision( self.player.character, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal, maxHp / collisionDamageMultiplier, 24, self.sv.velocityBuffer, self.sv.velocityBufferIndex, fallDamageMultiplier, g_disableFallDamage )
