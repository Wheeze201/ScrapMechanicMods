...
local function containerItemsTable( container )
	local items = {}

	for slot = 0, container:getSize() do
		local data = container:getItem( slot )
		print(data)
		data.uuid = tostring( data.uuid )
		items[ data.uuid ] = ( items[ data.uuid ] or 0 ) + data.quantity
	end

	return items
end

local function containerHasItems( container, required )
	local items = containerItemsTable( container )
	local missing = required
	local removed = {}

	for uuid, _ in pairs( required ) do
		if items[ uuid ] and missing[ uuid ] then
			if items[ uuid ] >= missing[ uuid ] then
				removed[ uuid ] = missing[ uuid ]
				missing[ uuid ] = 0
			else
				removed[ uuid ] = (removed[ uuid ] or 0) + items[ uuid ]
				missing[ uuid ] = missing[ uuid ] - items[ uuid ]
			end

			if missing[ uuid ] <= 0 then missing[ uuid ] = nil end
		end
	end

	if count(missing) == 0 then return true, removed
	else return false, missing end
end

local function containerRemoveItems( container, items )
	sm.container.beginTransaction()

	for uuid, quantity in pairs( items ) do
		print('spending ' .. quantity .. ' of ' .. uuid)
		sm.container.spend( container, sm.uuid.new( uuid ), quantity )
	end
...
function SurvivalGame.bindChatCommands( self )
	sm.game.bindChatCommand( "/build", { { "string", "name", false } }, "cl_onChatCommand", "Imports blueprint, but also requires resources, shows missing parts to you $SURVIVAL_DATA/LocalBlueprints/<name>.blueprint" )
	sm.game.bindChatCommand( "/destroy", {}, "cl_onChatCommand", "Destroys a creation and drops it's items (autosaves to $SURVIVAL_DATA/LocalBlueprints/destroyedAutosave.blueprint)" )
...
function SurvivalGame.cl_onChatCommand( self, params )
	...
	elseif params[1] == "/import" or params[1] == "/build" then
		local rayCastValid, rayCastResult = sm.localPlayer.getRaycast( 100 )
		if rayCastValid then
			local importParams = {
				world = sm.localPlayer.getPlayer().character:getWorld(),
	...
	elseif params[1] == "/destroy" then
		...
		if rayCastValid then
			...
			self.network:sendToServer( "sv_destroyCreation", importParams )
		end
...
function SurvivalGame.sv_exportCreation( self, params )
	...
	if success and existing then sm.json.save( existing, "$SURVIVAL_DATA/LocalBlueprints/"..params.name.."_backup.blueprint" ) print('This blueprint already exists! Backing up existing blueprint before exporting, to restore the original creation, use #00ff00/build ' .. params.name .. '_backup') end
...
function SurvivalGame.sv_importCreation( self, params )
	if params.consume then -- use resources from player inventory
		if not params.player then return error( "sv_importCreation: Argument 2 (params) is missing \"player\" field" ) end

		local success, data = pcall(sm.json.open, "$SURVIVAL_DATA/LocalBlueprints/"..params.name..".blueprint" )

		if not success then print('[qzqz] import error:', data) return self.network:sendToClients( "client_showMessage", "#ff0000Blueprint does not exist!" ) end

		local required = {}

		for _, body in pairs(data.bodies) do
			for _, child in pairs(body.childs) do
				local count = 1
				if child.bounds then count = child.bounds.x * child.bounds.y * child.bounds.z end
				if child.controller then
					if child.controller.container then
						for _, slot in pairs(child.controller.container) do
							if slot.quantity > 0 then	required[slot.uuid] = ( required[slot.uuid] or 0 ) + slot.quantity print(slot, required[slot.uuid]) end
						end
					elseif child.controller.containers then
						for _, container in pairs(child.controller.containers) do
							for _, slot in pairs(container.container) do
								if slot.quantity > 0 then	required[slot.uuid] = ( required[slot.uuid] or 0 ) + slot.quantity print(slot, required[slot.uuid]) end
							end
						end
					end
				end
				required[child.shapeId] = ( required[child.shapeId] or 0 ) + count
				print('added ' .. count .. ' of ' .. child.shapeId .. ' to required items')
			end
		end

		if data.joints then
			for _, joint in pairs(data.joints) do
				required[joint.shapeId] = ( required[joint.shapeId] or 0 ) + 1
			end
		end

		local inv = params.player:getInventory()

		local status, result = containerHasItems(inv, required)

		if status then
			containerRemoveItems(inv, result)
			sm.creation.importFromFile( params.world, "$SURVIVAL_DATA/LocalBlueprints/" .. params.name .. ".blueprint", params.position )
		else
			self.network:sendToClients( "client_showAlert", "You are missing items! Missing items printed to chat" )
			for uuid, quantity in pairs( result ) do
				self.network:sendToClients( "client_showMessage", "#ff0000" .. quantity .. " #ffff00" .. sm.shape.getShapeTitle(sm.uuid.new(uuid)) .. "#ffffff ( #00ffff" .. uuid .. "#ffffff )" )
			end
		end
	else
		sm.creation.importFromFile( params.world, "$SURVIVAL_DATA/LocalBlueprints/" .. params.name .. ".blueprint", params.position )
	...
	for _, body in pairs(data.bodies) do
		for _, child in pairs(body.childs) do
			...
			if child.controller then
				if child.controller.container then
					for _, slot in pairs(child.controller.container) do
						if slot.quantity > 0 then	items[slot.uuid] = ( items[slot.uuid] or 0 ) + slot.quantity print(slot, items[slot.uuid]) end
					end
				elseif child.controller.containers then
					for _, container in pairs(child.controller.containers) do
						for _, slot in pairs(container.container) do
			...
			items[child.shapeId] = ( items[child.shapeId] or 0 ) + count
		end
	end

	if data.joints then
		for _, joint in pairs(data.joints) do
			items[joint.shapeId] = ( items[joint.shapeId] or 0 ) + 1
		end
	end

	--[[ sm.container.beginTransaction()
	for uuid, quantity in pairs( items ) do
		sm.container.collect( params.player:getInventory(), sm.uuid.new(uuid), quantity )
	end
	sm.container.endTransaction() ]]

	--sm.event.sendToWorld( params.world, "sv_dropLoot", { position = params.position, items = items, player = params.player } )
	local loot = {}
	for uuid, quantity in pairs( items ) do
		table.insert(loot, { uuid = sm.uuid.new(uuid), quantity = quantity })
	end
	SpawnLoot(params.player, loot, params.player:getCharacter().worldPosition)

	for _, shape in pairs(params.body:getCreationShapes()) do print('destroying shape ' .. shape.id .. ' (' .. tostring(shape.shapeUuid) .. ')') sm.shape.destroyShape(shape) end

	self.network:sendToClients( "client_showMessage", "Accidentally lost a creation using /destroy? It was saved! It's LocalBlueprint name is autosave, you can build it again using #00ff00/build autosave" )
end
